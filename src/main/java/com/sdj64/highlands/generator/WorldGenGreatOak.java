...
public class WorldGenGreatOak extends WorldGenAbstractTree
{
    ...
    void generateLeafNodeList()
    {
        ...
        this.field_175948_j.add(new WorldGenGreatOak.FoliageCoordinates(this.blockPosOrigin.up(k), j));
            ...
            if (f >= 0.0F)
            {
                for (int l = 0; l < i; ++l)
                {
                    ...
                    if (this.func_175936_a(blockpos, blockpos1) == -1)
                    {
                        ...
                        {
                            this.field_175948_j.add(new WorldGenGreatOak.FoliageCoordinates(blockpos, blockpos2.getY()));
                        }
                    }
                }
            }
        ...
        for (int j = -i; j <= i; ++j)
        {
            for (int k = -i; k <= i; ++k)
            {
                if (Math.pow((double)Math.abs(j) + 0.5D, 2.0D) + Math.pow((double)Math.abs(k) + 0.5D, 2.0D) <= (double)(p_180712_2_ * p_180712_2_))
                {
                    ...
                    if (state.getBlock().isAir(world.getBlockState(blockpos1), this.world, blockpos1) || state.getBlock().isLeaves(world.getBlockState(blockpos1), this.world, blockpos1))
                    {
